...
export class CanvasClient {
  ...
  // 15. Download file contents (accepts pre-fetched metadata to avoid duplicate API call)
  async downloadFile(fileId: number, fileMeta?: CourseFile): Promise<{ buffer: Buffer; name: string; size: number }> {
...
if (require.main === module) {
  (async () => {
    ...
    if (!token) {
      console.error("Usage: CANVAS_TOKEN=xxx npx tsx src/canvas/client.ts");
      process.exit(1);
    }

    const client = new CanvasClient("https://ufv-es.instructure.com", token);

    try {
      console.log("=== Canvas Client Test ===\n");
      ...
      courses.forEach((c) => console.log(`    - [${c.id}] ${c.name} (${c.course_code})`));

      // Assignments of first course
      if (courses.length > 0) {
        const first = courses[0];
        const assignments = await client.getAssignments(first.id, true);
        console.log(`\n[+] Pending assignments for "${first.name}" (${assignments.length}):`);
        assignments.forEach((a) =>
          console.log(`    - ${a.name} | due: ${a.due_at ?? "no date"} | ${a.points_possible ?? 0} pts`)
        );

        // Grades
        try {
          const grades = await client.getGrades(first.id);
          console.log(`\n[+] Grades for "${first.name}":`, grades);
        } catch (e) {
          console.log(`\n[!] Could not get grades: ${(e as Error).message}`);
        }
      }
