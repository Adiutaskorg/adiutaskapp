...
function buildMessages(userMessage: string, history?: ConversationMessage[]): Anthropic.MessageParam[] {
  ...
  if (history && history.length > 0) {
    const trimmed = trimHistoryToTokenBudget(history, Math.floor(config.llmMaxTokens * 0.75));
...
const tools: Anthropic.Tool[] = [
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        only_pending: { type: "boolean", description: "Si true, solo devuelve tareas con entrega futura. Recomendado para consultas de pendientes." },
      },
      required: ["course_id"],
    },
  },
  ...
  },
  {
    ...
    description:
      "Lista los próximos eventos del calendario del estudiante (exámenes, entregas, clases especiales). Útil cuando preguntan '¿qué tengo esta semana?' o sobre eventos próximos.",
    input_schema: {
      type: "object" as const,
  ...
  },
  {
    ...
    description:
      "Obtiene los anuncios más recientes de todos los cursos del estudiante. Útil para novedades, avisos de profesores o cambios recientes.",
    input_schema: { type: "object" as const, properties: {}, required: [] },
  },
  {
    name: "get_course_files",
    description:
      "Lista los archivos más recientes subidos a un curso (apuntes, materiales, PDFs). Devuelve nombre, tamaño y fecha de actualización.",
    input_schema: {
      type: "object" as const,
      properties: { course_id: { type: "number", description: "ID del curso en Canvas" } },
      required: ["course_id"],
 ...
 * Strips unnecessary fields and limits array sizes.
 */
function compressToolResult(toolName: string, rawJson: string): string {
  try {
    ...
    switch (toolName) {
      ...
      case "get_grades": {
        // Grades are already compact, pass through
        return rawJson;
      }
...
async function getCachedCourses(canvas: CanvasClient): Promise<Course[] | undefined> {
  try {
    // getCourses() uses internal cache, so this is cheap if already cached
          ...
          for (const block of response.content) {
            ...
            try {
              ...
              if (err instanceof TokenExpiredError) {
                return "⚠️ Tu token de Canvas ha expirado o no es válido. Renuévalo en Canvas (Perfil > Configuración > Tokens de acceso) y actualízalo con /vincular.";
              }
              toolResults.push({
                type: "tool_result",
...
export class GeminiProvider implements LLMProvider {
  async processMessage(_message: string, _canvas: CanvasClient, _history?: ConversationMessage[]): Promise<string> {
    return "⚠️ Gemini no está implementado todavía.";
  }
}
...
export function createLLMProvider(): LLMProvider | null {
  ...
  if (geminiKey) return new GeminiProvider();
